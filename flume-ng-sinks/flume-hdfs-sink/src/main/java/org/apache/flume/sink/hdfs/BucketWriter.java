...
class BucketWriter {
    ...
    BucketWriter(long rollInterval, long rollSize, long rollCount, long batchSize, Context context,
	    String filePath, String fileName, String inUsePrefix, String inUseSuffix,
	    String fileSuffix, CompressionCodec codeC, CompressionType compType, HDFSWriter writer,
	    ...
	    String onCloseCallbackPath, long callTimeout, ExecutorService callTimeoutPool,
	    long retryInterval, int maxCloseTries) {
     ...
     */
    private <T> T runPrivileged(final PrivilegedExceptionAction<T> action) throws IOException,
	...
	return new Callable<Void>() {
		...
		try {
		    ...
		    LOG.warn("Closing file: " + path + " failed. Will " + "retry again in "
			    + retryInterval + " seconds.", e);
     ...
     * @throws InterruptedException
     */
    public synchronized void close(boolean callCloseCallback) throws IOException,
	    InterruptedException {
	checkAndThrowInterruptedException();
	...
	if (!isBatchComplete()) {
		...
		if (idleFuture == null || idleFuture.cancel(false)) {
		    ...
		    idleFuture = timedRollerPool
			    .schedule(idleAction, idleTimeout, TimeUnit.SECONDS);
		}
	    }
	}
    }
     ...
     */
    private <T> T callWithTimeout(final CallRunner<T> callRunner) throws IOException,
	    InterruptedException {
	...
	try {
	    ...
	    future.cancel(true);
	    sinkCounter.incrementConnectionFailedCount();
	    throw new IOException("Callable timed out after " + callTimeout + " ms" + " on file: "
		    + bucketPath, eT);
