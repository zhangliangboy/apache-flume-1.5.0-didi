...
public class TimeBasedIndexNameBuilder implements IndexNameBuilder {
    ...
    public String getIndexName(Event event) {
        ...
        String actualIndexPrefix = null;
        String projectName = timestampedEvent.getHeaders().get("projectName");
        if (projectName == null || projectName.isEmpty() || projectName.equalsIgnoreCase("null")) {
            actualIndexPrefix = indexPrefix;
        } else {
            actualIndexPrefix = projectName;
        }
        return new StringBuilder(actualIndexPrefix).append('-').append(fastDateFormat.format(timestamp)).toString();
    }
