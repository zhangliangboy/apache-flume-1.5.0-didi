 ...
 * server (such as Flume's {@link org.apache.flume.source.ThriftSource}).
 * </p>
 * <p>
 ...
 * the RPC implementation encapsulated in {@link RpcClient}. The destination is
 * an instance of Flume's {@link org.apache.flume.source.ThriftSource}, which
 ...
 * batches of the configured <tt>batch-size</tt>. The batch size has no
 * theoretical limits although all events in the batch <b>must</b> fit in
 * memory. Generally, larger batches are far more efficient, but introduce a
 * slight delay (measured in millis) in delivery. The batch behavior is such
 * that underruns (i.e. batches smaller than the configured batch size) are
 * possible. This is a compromise made to maintain low latency of event
 * delivery. If the channel returns a null event, meaning it is empty, the batch
 * is immediately sent, regardless of size. Batch underruns are tracked in the
 * metrics. Empty batches do not incur an RPC roundtrip.
 * </p>
 * <p>
