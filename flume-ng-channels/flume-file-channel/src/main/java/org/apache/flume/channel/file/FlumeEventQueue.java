 ...
 * {@link FlumeEventPointer} objects which are represented as 8 byte longs
 * internally. Additionally the queue itself of longs is stored as a memory
 * mapped file with a fixed header and circular queue semantics. The header of
 * the queue contains the timestamp of last sync, the queue size and the head
 * position.
 */
final class FlumeEventQueue {
	...
	db = DBMaker.newFileDB(dbFile).closeOnJvmShutdown().transactionDisable()
		.syncOnCommitDisable().deleteFilesAfterClose().cacheDisable().make();
	...
	if (!backingStore.syncRequired() && !inflightTakes.syncRequired() && !force) { // No
										       // need
										       // to
										       // check
										       // inflight
    ...
    /**
     * Add a FlumeEventPointer to the head of the queue. Called during
     * rollbacks.
     * 
     * @param FlumeEventPointer
     *            to be added
     * @return true if space was available and pointer was added to the queue
     */
    synchronized boolean addHead(FlumeEventPointer e) {
	// Called only during rollback, so should not consider inflight takes'
	// size,
	...
	// in such a way that these takes cannot go back in. If this if returns
	// true,
    ...
    /**
     * Must be called when a put happens to the log. This ensures that put
     * commits after checkpoints will retrieve all events committed in that txn.
     * 
     * @param e
     * @param transactionID
     */
    ...
    /**
     * Remove FlumeEventPointer from queue, will only be used when recovering
     * from a crash. It is not legal to call this method after replayComplete
     * has been called.
     * 
     * @param FlumeEventPointer
     *            to be removed
     * @return true if the FlumeEventPointer was found and removed
     */
     ...
     *         will be normally be used when deciding which data files can be
     *         deleted
     */
	...
	if (index < 0 || index > backingStore.getSize() - 1) {
	    throw new IndexOutOfBoundsException("index = " + index + ", queueSize "
		    + backingStore.getSize() + " " + channelNameDescriptor);
	}
	copyCount++;
	long start = System.currentTimeMillis();
    ...
    /**
     * Called when ReplayHandler has completed and thus
     * remove(FlumeEventPointer) will no longer be called.
     */
    synchronized void replayComplete() {
	String msg = "Search Count = " + searchCount + ", Search Time = " + searchTime
		+ ", Copy Count = " + copyCount + ", Copy Time = " + copyTime;
	LOG.info(msg);
	if (db != null) {
	    db.close();
	}
	...
	// Both these are volatile for safe publication, they are never accessed
	// by
	...
	public InflightEventWrapper(File inflightEventsFile) throws Exception {
		...
		Preconditions.checkState(inflightEventsFile.createNewFile(), "Could not"
			+ "create inflight events file: " + inflightEventsFile.getCanonicalPath());
	    }
	...
	public void serializeAndWrite() throws Exception {
	    ...
	    // transaction id number of events for this txn list of
	    // eventpointers

	    try {
		int expectedFileSize = (((inflightEvents.keySet().size() * 2) // For
									      // transactionIDs
									      // and
									      // events
									      // per
		...
		+ 16; // Checksum
		// There is no real need of filling the channel with 0s, since
		// we
		...
		for (Long txnID : inflightEvents.keySet()) {
		    ...
		    LOG.debug("Number of events inserted into " + "inflights file: "
			    + String.valueOf(pointers.size()) + " file: "
	 ...
	 * {@link com.google.common.collect.SetMultimap} of transactionIDs to
	 * events that were inflight.
	 * 
